...
- The receiver's acknowledgement can be on ANY page of the file, not only the bill of lading: a packing list, a pick slip, a delivery receipt, the back of a page. Distribution-centre and grocery receivers often ink a receiving block ACROSS the packing list - rotated sideways and printed over the text, with fields such as "Distributors Receiving", "Pallets Off Loaded", "Shrink Wrap Intact", "# Cases", "Date" and "Signature". Look at every page, in every orientation, for one. When any page carries the consignee's signature or receiving stamp: set stamp_present (and receiver_signed and receiver_name when it is signed), give THAT page role "pod" in pages[] whatever form it is printed on, set document_type to proof_of_delivery, and say in notes which page carries it. Load 2590747: a KeHE "DC 12 Distributors Receiving" stamp, signed, with 21 pallets off-loaded, sat sideways on page 1 of the packing list while the BOL behind it was unsigned - the file is the POD.
- Costco receiving stickers are proofs of delivery. Costco's distribution centres do not sign the BOL: the receiver prints a label or thermal receipt for the driver - often printed twice on one strip, stuck on or laid over the BOL - reading like "DOOR 445 / MONROE TOWNSHIP DR 9/25/26 / APP TIME 07:30 / ARR TIME 04:20 / IN TIME 07:59 / OUT TIME 08:32 / RECVR: G QUI / TRAILER LEAVING EMPTY", with the PO and a barcode. Other distribution centres print the same kind of label. That label on its own IS the proof of delivery: set document_type proof_of_delivery, stamp_present true, receiver_name to the RECVR value, receiver_date to the label's date, check_in / check_out from IN TIME / OUT TIME with at_stop "consignee", and give its page role "pod". When the label is legible, be as sure of it as of a signed POD - a BOL half hidden underneath does not make the file less certain. A photo of a dock pager or a check-in screen is not a receipt; it is a photo. Load 2572625 (25 Sep 2026): a legible Costco sticker was read at 83% and held.
...
- For every page in pages[], give doc_ref: the page's own document number as printed in its header - the order, BOL or delivery number - and its page number, written as "<number> <page>/<pages>", for example "P2700C 1/3"; null when the page prints neither. Give signed true when the page carries any handwritten signature, handwritten mark or stamp. Drivers often scan the same pages several times into one file: every copy of a page gets the same doc_ref.
...
- Origin paperwork signed by the DRIVER when collecting the freight (a CFS or warehouse "delivery" or release receipt, gate pass, pickup receipt, an airline or forwarder delivery order) is pickup paperwork: document_type bill_of_lading, and say in notes what the form is.
- A shipper's own loading document used in place of a straight bill of lading - a Warehouse Pick Slip, pick ticket, loading sheet or shipping order that carries the load or order number, the customer PO and the ship-to, and is handed to the driver at pickup - is document_type bill_of_lading, not shipping_document. So is its sign-out side photographed on its own (driver signature and printed name, carrier, seal, trailer, check-in and leaving times, shipper's initials). Say in notes which side of the form it is. Load 2576831: a City Brewing Warehouse Pick Slip - load number and customer PO on the front, the driver sign-out on the back - was the shipment's BOL. proof_of_delivery needs the CONSIGNEE's signature, stamp or printed name at the destination; a signature line filled at the origin does not make a POD, and "delivery" in a form's title does not either.
...
- pod: a bill of lading, delivery receipt, packing list or any other shipment page that the RECEIVER (consignee) has signed, stamped, or written received / in / out times on at the delivery. Receivers often stamp a receiving block sideways across a packing list ("Distributors Receiving", "Pallets Off Loaded", "Signature"). A Costco receiving sticker or thermal receipt (DOOR, APP / ARR / IN / OUT TIME, RECVR, "TRAILER LEAVING EMPTY") is a pod, even laid over an unsigned BOL. If ANY page of the file is like this, answer pod.
- bol: a bill of lading (straight, short form, pickup copy, continuation or detail page) with nothing from the receiver on it. A shipper or driver signature alone is still bol. A shipper's warehouse pick slip or loading sheet handed to the driver, and its sign-out side (driver signature, seal, trailer, times), count as bol.
...
- photo: a picture of freight, pallets, a trailer, a seal, a truck, a dock, a dock pager or a check-in screen.
